    ...
    <div className="min-h-[70vh] grid place-items-center px-4">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm bg-white dark:bg-zinc-900 rounded-2xl shadow-soft p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <div>
            <h1 className="text-xl font-semibold">Create your cozy space ✨</h1>
            <p className="text-sm text-zinc-500">Let’s start something lovable.</p>
          </div>
        </div>
        <form onSubmit={onSubmit} className="space-y-3">
          <div>
            <label className="block text-sm mb-1">Name</label>
            <input className="w-full rounded-lg border-zinc-200 dark:border-zinc-800 bg-transparent" value={name} onChange={e=>setName(e.target.value)} required />
          </div>
          <div>
          ...
          <button type="submit" className="w-full rounded-lg bg-gradient-to-tr from-blush-400 to-blush-600 text-white py-2">Sign up</button>
        </form>
        <p className="text-xs text-zinc-500 mt-3">Already have an account? <Link to="/login" className="text-blush-500">Login</Link></p>
      </motion.div>
    </div>
  )
}